...
- [ ] [INPUT — meeting] [INBOUND routed:b0c5 from loderite] Investigate the T-matrix (EM-scattering) ↔ Gaussian-splat ↔ WiRoHSH (Wick-rotated hyper-spherical harmonics) relationship — can a precomputed per-geometry response basis (T-matrix analogy) represent Gaussian splats compactly via WiRoHSH? loderite may become a showcase if applicable. **INDEPENDENCE AUDIT 2026-09-07 (`docs/dreamed/fig-wirohsh.md`, surfaced not decided):** the "five grounds" framing overstates the evidential weight. Only TWO of the five WiRoHSH essays actually bear on id:ff32 (refraction explicitly declines to rule, ladder defers and borrows by citation, discontinuities never mentions the item and is a RECOVERY result, opposite in polarity). The five grounds collapse to TWO roots: the splat pipeline contains no wave or scattering problem (breaks 1 and 2), and an analytic basis cannot localize (breaks 3, 4, 5). The merge is SELF-REPORTED at `docs/dreamed/wirohsh-approximation.md:187`, "the domain restriction and the analyticity restriction are the same restriction wearing two hats", not an auditor's inference. The two roots are genuinely independent of each other and either alone would close the item, so the RECOMMENDATION is unaffected and only the rhetoric of five-fold convergence is. Third instance this session of a convergence claim collapsing under checking. <!-- id:ff32 -->
...
- [ ] [INBOUND routed:59e7 from mathematical-writing] ESSAY CANDIDATE (owner-authored, flagged not auto-written — narrative is the owner's domain per the working contract; same terms as mathematical-writing id:aae4's gtnsd essay flag): adapt the "Bloch Truth" material into an essays/ piece. **UNBLOCKED 2026-09-07 (owner ruling, `docs/meeting-notes/2026-09-07-1508-bloch-truth-rulings.md` D2).** The missing thesis now exists and is RATIFIED: *"Bloch Truth adds a provability reading to a geometry already published by Sperling and Walmsley (2018)"* -- their Phys. Rev. A **97**, 062327 section IV.3 *"True, false, and undecidable"* already places true/false at the Bloch poles, an undecidable continuum at the equator and a double cone as the classical convex hull, with NO provability predicate anywhere in it, so the logic is the new part and the picture is not. Authorship terms UNCHANGED: this stays owner-authored and must NOT be auto-written. Superseded blocker was: — the 2026-07-17 .mw meeting found no thesis statement exists in 412 session files, and an essay cannot be written on a non-thesis. Origin quote: knowledge/sessions/claude-ai/2025-08-05_breaking_project_paralysis_cbae6cd6.md:1334 — "the Bloch Truth (might need a better name) might be useful for the AI logic core in the second (ZFC?) layer where incompleteness applies (core layer should only be complete, e.g. ZF without C)". <!-- id:4bb2 -->
...
- [ ] [OWNER-RATIFIED 2026-09-07] **RECURRING PRACTICE: run an ELI12 pass every now and then, and feed it back into the adult results.** The 2026-09-07 poster batch established that writing the plain-language version is not only outreach, it is a **compression test on the argument itself**: if a claim will not compress, the core of it has usually not been found yet, and the plain phrasing is often the better ADULT phrasing. Four concrete instances from that batch, each an improvement on prose that had already been through review: (1) *"Four ways in. Two ways out. Something got lost."* states the ancilla counting argument better than `logic-bloch-gates.md` does, and is checkable by counting arrows; (2) the scissors result drawn as an EMPTY WEDGE labelled *"you would want to be up here. Nobody is."* is more legible than `fig-results.md`'s two-panel version, because it names the absent regime rather than asking the reader to compare two curves; (3) conservativity as a **two-storey house with a solid floor** (borrow all you like upstairs, downstairs proves exactly what it did before) beats the technical panel in `fig-boundary-and-big-five.md`; (4) drawing the playground Goedel (*"this sentence is a lie"*) NEXT TO the real one (*"this cannot be proved"*) forces precision about what the theorem actually claims, which the essays state correctly but never make vivid. **How to apply:** after a cluster of work settles, do an ELI12 pass over its headline claims, then carry any sharper formulation BACK into the adult text rather than leaving it in the kids' version. Treat a claim that resists plain statement as a signal the claim is not yet understood, not as a sign the material is too advanced. Cadence is deliberately loose ("every now and then", owner's words), not per-session. **Owner's framing, same day:** this is the *"sorry for the long essay, I didn't have time to shorten it"* adage (Pascal, *Lettres provinciales*, Letter XVI, 4 December 1656, verified; routinely misattributed to Twain, T. S. Eliot and Cicero). That supplies the MECHANISM the four instances only demonstrate: **length is the cheap default and brevity is the expensive one**, because compressing forces you to decide what the claim actually is. So the ELI12 pass is the shortening labour done deliberately, and the understanding is produced BY that labour, which is why the sharper phrasing must be carried back into text written before that work was done. **Corroborated by this batch's own failure mode:** every essay and figure overran its size brief (39 to 81 KB against 20 to 32 KB asked), each agent giving the same reason, that cutting further meant dropping content. Nobody had time to make it shorter. Treat a systematic size overrun as a signal that the compression work is OUTSTANDING, not as a harmless style miss. <!-- id:a1aa -->
...
- [ ] **[OWNER] Triage the dreamed Bloch Truth cluster, SECOND WAVE** (`docs/dreamed/`, 9 more essays + 8 Lean files, 2026-09-07) <!-- id:352a -->
  ...
  - **Owner-originated, credited:** the mixed-state question did real work -- it dissolved the
    ...
    completeness.
- [ ] **[OWNER] Triage the dreamed Bloch Truth cluster** -- **PARTLY RULED 2026-09-07, see `docs/meeting-notes/2026-09-07-1508-bloch-truth-rulings.md` (D1 ball demoted to exposition, the report triangle is the object and the azimuth is gauge; D2 thesis ratified and 4bb2 unblocked; D3 prototype ordered; D4 three follow-ups adopted). Reinstating the ball AS THE OBJECT is deliberately kept OPEN and filed for later: it becomes available only if someone names a pair of questions about a sentence that cannot be answered simultaneously, which nine invented candidates failed to do. The remaining findings in this batch are still untriaged.** (`docs/dreamed/`, 4 essays + 4 Lean files, 2026-09-07) **The two located findings in owner material were FILED into `REVIEW_ME.md` 2026-09-07 on the owner's explicit instruction (`id:251e` ZF-without-C, `id:4787` the `(2^2)!` slip): surfaced and queued for his ruling, still NOT corrected.** <!-- id:c454 -->
...
- [x] [ROUTINE] **Lean drift guard for the 56 `docs/dreamed/lean/` files: pinned-good hashes + re-check on touch, AND a pinned Mathlib version/hash (owner ruling 2026-09-08, `/relay human`).** Problem: those 56 files sit OUTSIDE `make test` (`verify/lakefile.toml` targets only `Resogram`), so a Mathlib bump could silently invalidate 56 already-PUBLISHED machine-checked claims with nothing to catch it. The owner rejected nightly re-verification on cost and chose the cheap tier -- **but explicitly added the Mathlib pin, and that addition is what makes the cheap tier sound.** Pinned hashes alone re-check only when a file is touched, so an upstream Mathlib change would go unnoticed indefinitely; pinning the toolchain removes the drift at the source rather than detecting it after the fact, so the two halves are one mechanism and MUST ship together. **Acceptance:** (a) a recorded good hash per file; (b) re-verification fires when a file OR the Mathlib/toolchain pin changes -- the pin is a trigger, not just documentation; (c) the pinned Mathlib version is recorded where a reader of the published claims can find it, since the claim being made is 'machine-checked', which is only true relative to a stated toolchain. Do NOT close this with hashes and no pin, or with a pin and no trigger. <!-- relates:ef6b --> <!-- id:0720 -->
- [ ] [INPUT - decision] **Three PUBLISHED content errors found 2026-09-08 (`/relay human`) -- owner FILED them and chose to handle the 10 tier-(c) items first; the fix wording is authorial and his alone.** Deliberately not fixed by an agent: this is a public site whose claim is rigour, so the correcting clause is the owner's to write. (1) `physics/toesnail.md:79` states complex numbers cannot be ordered, then `:89` invokes `<Psi|Psi> > 0` without first establishing via conjugate symmetry that the quantity is REAL -- an ordering claim applied to something not yet shown orderable; this also reappears downstream as a Lean typing obligation. (2) `physics/entropy.md:59`'s bosonic step is valid only on Lambert-W branch `W_-1`, NOT the default `W_0` that every library ships -- numerically 1.8e-25 against 0.1 at beta*E_1 = 0.1, so a reader reproducing it with a stock library gets a wrong answer silently. Ties to the already-ratified marker split and the Lean queue `id:37cc`. (3) `physics/acoustics.md` gives `c^2 = gamma*p/rho` without stating the ADIABATIC assumption; isothermal gives 290 m/s against the adiabatic 343 m/s, an 18% discrepancy, and this is the one error a non-specialist reader could actually act on. Each needs one clause, not a rewrite. <!-- id:e029 -->
- [ ] [INPUT - meeting] **Route the WHOLE dreamed Bloch Truth cluster to `/meeting` -- in a DEDICATED session, deliberately NOT now (owner ruling 2026-09-08, `/relay human`).** The owner chose a full session over answering the open piece in passing, and the not-now is part of the ruling: do not let a later sweep pick this up as a quick call. **Agenda, all three together:** (1) reinstating THE BALL AS THE OBJECT, the piece deliberately left open by the 2026-09-07 rulings (which demoted the ball to exposition, made the report triangle the object and the azimuth gauge, ratified the thesis and unblocked `id:4bb2`); (2) whether the four dreamed essays actually unblock `id:4bb2`, which this ledger still records as BLOCKED on a 2026-07-17 `.mw` finding that predates them; (3) the prior question both of those rest on -- **whether AI-generated, owner-seeded, UNREVIEWED essays can serve as an unblocker at all.** That third point is why this is a session and not a decision: the four essays and four Lean files were produced by delegated agents and nothing was filed into any ledger, precisely because a delegated verdict is a recommendation and never self-settling. The Lean files were re-verified by the coordinator rather than merely reported (`Weltformel`, `LogicBloch`, `LogicGates`, `LogicQutrit`: exit 0, zero `sorry`, run capped under `docs/dreamed/capped.sh`), so the FORMAL content stands; the ESSAY content has still never been read by the owner. Index: `docs/dreamed/README.md`. <!-- relates:4bb2 --> <!-- id:e562 -->
- [ ] **[OWNER] Two decisions surfaced by the 2026-09-08 carry-back pass** (`id:a1aa` practice run, commit `c07489e`). 8 of `docs/dreamed/pascalized.md`'s 10 carry-backs and all four ELI12 poster wins were applied; the new advisory tier `tests/test_carryback.sh` now reports 8 present, 0 missing, 2 refused. The two refusals are the decisions. **(1) The carry-back list cannot express a MERGED distillation.** Item 2's sentence ("A core that cannot name a proof is a dashboard, not a supervisor") is a merge of two essays -- `pascalized.md`'s own ladder entry says "logic-layered-core §4.4, **with** counterfactual-boundary §6.4" -- but the item names only `logic-layered-core.md` as target. That essay calls the kernel a supervisor eight times (l.176, l.603, l.648, l.990) and argues inexpressiveness is what lets it safely REMAIN one; "dashboard, not a supervisor" is `logic-counterfactual-boundary.md:572`, reached by a stronger prior-stage argument (a core that cannot interpret arithmetic cannot represent a proof-as-sequence at all). Applying it to the parent alone would plant a sibling's conclusion in an essay whose text contradicts it. **Ruling needed:** should a merged distillation name both targets, carry a per-essay formulation each, or be dropped from the carry-back list as un-carriable? **(2) `wirohsh-ladder.md` §5 has no short form yet.** Item 8's one-liner drops the degrees-of-freedom counting proof that grounds the verdict (l.239), the locality obstruction as a second and separate cost (l.253), and the explicit radial-case exception (l.261) where a rung DOES pay -- turning a qualified structural finding into an unqualified one. This is the list's one distillation error, caught at carry-back rather than propagated. **Ruling needed:** is a shorter form that keeps the radial exception worth hunting, or does §5 stay at three paragraphs? Both refusals are recorded inline in `pascalized.md` with their reasons, so the tier reports them as discharged rather than as outstanding debt. <!-- id:ed2b -->
- [ ] **[OWNER] Read `docs/dreamed/pascalized.md` as the cheap route through the six open dreamed-triage items** (22 KB, ~15 min; site: `/dreamed/pascalized`). It reduces all 58 dreamed essays to one claim each (not one topic each), and where a claim did not survive later checking the same sentence says so. `id:2460`, `id:ff4c`, `id:e50c`, `id:352a`, `id:c454` and `id:6646` are all [OWNER] triage items pointing into that same tree, and each currently asks you to read essays to rule on them; pascalized is the compressed entry point that makes those rulings possible without the essays. Offered as a route, not a substitute: a ruling on any specific finding should still check that finding's own essay. NOT filed here: the located findings in owner material and the recommendations awaiting ratification, which already live in `REVIEW_ME.md` (`:69-178` and `:180+` respectively) and are deliberately not duplicated into this ledger. <!-- id:781e -->
- [ ] **[OWNER] Rule on the Tarski-ranking disagreement between two dreamed essays** -- surfaced by the 2026-09-08 pascalized audit (commit `5e0dd2c`), which found it reported as SETTLED in the summary when both essays leave it open. `docs/dreamed/omniscience.md` §2 ranks Tarski undefinability "Nearly" applicable, below Cantor/Lawvere, on the grounds that Tarski still needs a formal language, which the ket 42 has not been given. `docs/dreamed/weltformel-impossibility.md:328-332` argues that ranking is right for the ket 42 and MISLEADING for a world formula, because Tarski's hypotheses are strictly weaker than Goedel I's on the axis that matters: Tarski does not need effective axiomatizability, so it survives where Goedel I does not. That essay files the clash itself at `:501-504` as "Disagreement with a sibling essay ... Ruling needed: whether the sibling's table gets a qualifying sentence, or this ranking is rejected." **Both readings are internally consistent and the disagreement is about SCOPE (which object is being ranked), not about the theorems**, so it is a genuine owner call and not a defect to fix. Consequence if left open: the Tarski-vs-Goedel-I strength claim is load-bearing for the weltformel essay's one surviving impossibility verdict (no self-certification), which is the finding that batch would most want ratified. <!-- id:52ea -->
## Done
